...
async def validation_exception_handler(request: Request, exc: ValidationError):
    ...
    return JSONResponse(
        status_code=200,
        content=StdErrorRes(
            code=ErrorCode.ParamsError.code, message=exc.__str__()
...
async def base_exception_handler(request: Request, exc: StdError):
    ...
    return JSONResponse(
        status_code=200,
        content=StdErrorRes(code=exc.code, message=exc.msg).model_dump(mode="json"),
    )
...
async def exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=200,
        content=StdErrorRes().model_dump(mode="json"),
    )
